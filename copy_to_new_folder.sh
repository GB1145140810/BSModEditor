...
if [ ! -f "$TARGET_DIR/app/src/main/res/values/colors.xml" ]; then
    cat > "$TARGET_DIR/app/src/main/res/values/colors.xml" << 'EOF'
<?xml version="1.0" encoding="utf-8"?>
<resources>
...
# 创建占位图标文件
if [ ! -f "$TARGET_DIR/app/src/main/res/mipmap-hdpi/ic_launcher.png" ]; then
...
# 生成构建脚本
echo "8. 生成构建脚本..."
cat > "$TARGET_DIR/build_optimized.sh" << 'EOF'
#!/bin/bash
...
chmod +x "$TARGET_DIR/build_optimized.sh"
...
echo "2. 运行构建脚本: ./build_optimized.sh"
echo "3. 或者使用Android Studio打开项目"
echo ""
echo "注意事项:"
echo "1. 确保已安装JDK 8+"
echo "2. 确保Android SDK已配置"
echo "3. 如果构建失败，考虑使用GitHub Actions"